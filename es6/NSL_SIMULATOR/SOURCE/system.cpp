...
void System :: step(){ // Perform a simulation step
  ...
  else for(int i=0; i<_npart; i++) this->move(int(_rnd.Rannyu()*_npart)); // Perform a MC step on a randomly choosen particle (provo _npart volte)
...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    bool up= r < (1./(1.+exp(-2.*_beta*(_J*double(_particle(this->pbc(i-1)).getspin()+_particle(this->pbc(i+1)).getspin()) + _H ))) ); // anche nell'algoritmo di metropolis c'è il +_H
    
    //k_b=1
    if(up)  _particle(i).setspin(1);
...
void System :: measure(){ // Measure properties
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut){
          ...
          if(_measure_pressure) pres_temp+= 1.0/pow(dr,12) - 0.5/pow(dr,6);
  ...
  if (_measure_temp and _measure_kenergy){
      ...
      temper=_measurement(_index_temp);
  } 
  // PRESSURE //////////////////////////////////////////////////////////////////
  if(_measure_pressure){
    _measurement(_index_pressure)=_ptail+_rho*temper+pres_temp*48./(3.*_volume);
    // cout << temper << endl; 
  }
...
// TO BE FIXED IN EXERCISE 6
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  // if(_measure_cv and _measure_tenergy) _measurement(_index_cv)=pow(_measurement(_index_tenergy),2); 
  // // in questa variabile salvo H^2 (con H energia media per particella)
  if(_measure_cv and _measure_tenergy) _measurement(_index_cv)=pow(_measurement(_index_tenergy)*_npart,2); 
  // in questa variabile salvo H^2 (con H energia totale)


// TO BE FIXED IN EXERCISE 6

  // if(_measure_magnet and _measure_chi) _measurement(_index_chi)=pow(_measurement(_index_magnet),2);
  // // in questa variabile salvo M^2 (con M magnetizzazione)
  if(_measure_magnet and _measure_chi) _measurement(_index_chi)=_beta*pow(_measurement(_index_magnet),2);
...
void System :: averages(int blk){ //questa funzione la chiamo alla fine di un blocco di misure

  ofstream coutf;
  coutf << scientific;
  double average, sum_average, sum_ave2;

  _average     = _block_av / double(_nsteps); //vettore con media dell'ultimo blocco per ogni proprietà
  // if(_measure_cv and _measure_tenergy){
  //   _average(_index_cv)=pow(_beta*_npart,2)*(_average(_index_cv)-pow(_average(_index_tenergy),2)); //il fattore _npart è dovuto al fatto che in: _measurement(_index_tenergy) io ho l'energia di una singola particella
  //   // qui ho il vero valore di cv dell'ultimo blocco
  // }
  if(_measure_cv and _measure_tenergy){
    _average(_index_cv)=pow(_beta,2)*(_average(_index_cv)-pow(_npart*_average(_index_tenergy),2)); //il fattore _npart è dovuto al fatto che in: _measurement(_index_tenergy) io ho l'energia di una singola particella
    // qui ho il vero valore di cv dell'ultimo blocco
  }
  // if(_measure_magnet and _measure_chi){
  //   _average(_index_chi)=_beta*(_average(_index_chi)-pow(_average(_index_magnet),2)); 
  //   // qui ho il vero valore di chi dell'ultimo blocco
  // }
