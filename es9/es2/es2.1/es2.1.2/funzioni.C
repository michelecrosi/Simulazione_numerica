...
lin::lin(const double& co_ang, const double& inter){
    m=co_ang;
    q=inter;
}
lin::~lin(){
...
void lin::Setm(const double& co_ang){
    m=co_ang;
}
void lin::Setq(const double& inter){
    q=inter;
}
double lin::Getm() const{
    return m;
}
double lin::Getq() const{
    return q;
}
double lin::f(const double& x) const{
	return m*x+q;
}
void lin::normalizza(const double& min, const double& max){
    double N=1./((max*max-min*min)*0.5*m+(max-min)*q);
    m*=N;
    q*=N;
}
