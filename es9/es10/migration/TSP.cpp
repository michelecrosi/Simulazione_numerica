...
_operator::_operator(int riga) : rnd(riga){
	esponent=2; // messo maggiore di 1 per avere maggiore estrazione vicino a 0
	// è l'esponente usato per la selezione
}
...
// usato per esercizio 10 (in file con città ho 2 argomenti)
TSP::TSP(int riga, string input_file_TSP, string input_file_cities, std::function<double(vector<double>& , const vector<double>& )> input_distance) : op(riga), distance(input_distance){
	int n_dim=2; // numero di cordinate di ogni città
	vector<double> posit(n_dim); // posizione
	vector<vector<double>> V_pos;// vettore in cui salvo le posizioni 
	ifstream input;
	...
	while ( !input.eof() ){
		...
		if( prop == "P_m" ){
			...
			N_migr=int(property);
			if(N_migr <= 0){
				cerr << "PROBLEM: number of generations between two migrations must be positive" << endl;
			}
		} else if( prop == "ENDINPUT" ){
			break;
		} else{
...
void TSP::Set_N_migr(const unsigned int& N_mig){
	if(N_mig <= 0) cerr << "ERRORE! Il numero di generazioni tra due migrazioni non può essere negativo\n";
}

unsigned int TSP::Get_N_migr(){
	return N_migr;
}