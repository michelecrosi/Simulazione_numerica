...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    bool up= r < (1./(1.+exp(-2.*_beta*(_J*double(_particle(this->pbc(i-1)).getspin()+_particle(this->pbc(i+1)).getspin()) + _H ))) ); // nell'algoritmo di metropolis c'è anche il +_H
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _ptail =32.*M_PI*_rho*_rho*(1./(9.*pow(_r_cut,9))-1./(6.*pow(_r_cut,3)));
        // TO BE FIXED IN EXERCISE 7
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr and dr<semilung){ // se non controllo ho r che può raggiungere fino a sqrt(3) volte questo valore
          int bin=floor(dr/double(_bin_size));
          // cerr << bin << endl;
          _measurement(_index_gofr+bin)+=2; //indice del bin da incrementare
        }
  ...
  //devo stare attento che devo misurare sempre la temperatura se voglio misurare la pressione a partire da quella misura
...
// TO BE FIXED IN EXERCISE 6
  ...
  // in questa variabile salvo H^2 (con H hamiltoniana)
// TO BE FIXED IN EXERCISE 6
...
void System :: averages(int blk){ //questa funzione la chiamo alla fine di un blocco di misure
  ...
  if(_measure_gofr){
    for(int i=0;i<_n_bins;i++){
      double r=(static_cast<double>(i)+0.5)*_bin_size; // usato il valor medio del bin
      double DV=4.*M_PI*(pow(r+_bin_size,3)-pow(r,3))/3.;
      _average(_index_gofr+i)/=(_rho*_npart*DV);
    }
  }
