...
double Random::lineare(const lin& l,double min, double max){
   // perchè questa funzione funzioni è necessario che la funzione sia a segno definito
   if(l.f(min)*l.f(max)<0) cerr << "ERRORE! la funzione inserita in Random::lin(lin l,double min,double max) deve errere a segno definito in: [min,max)";
   double coef=l.Getm();
   double inter=l.Getq();
   double N=1./((max*max-min*min)*0.5*coef+(max-min)*inter); // fattore che normalizzazione da moltiplicare alla funzione per trasformarla in una distribuzione in [min,max)
   // in questo codice gli do già in imput la distribuzione normalizzata
   ...
   // coef*=N;
   // inter*=N;
   return (-inter+sqrt((inter+coef*min)*(inter+coef*min)+2*coef*Rannyu()/N))/coef;
}
